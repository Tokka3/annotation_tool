    ...
    for (const auto& entry : std::filesystem::directory_iterator(annotation_path)) {
        if (entry.is_regular_file()) {
            ...
            if (path.extension() == ".png") {
                ...
                if (D3DXCreateTextureFromFileA(g_pd3dDevice, image_path.c_str(), &img.texture) != D3D_OK) {
                    std::cerr << "Failed to load texture from file: " << image_path.c_str() << std::endl;
...
bool key_pressed(const int key) {
    if (GetForegroundWindow() != FindWindowA(0, "Nigger")) {
        return false;
    }
...
int main(int, char**)
{
    ...
    ::RegisterClassExW(&wc);
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Nigger", WS_OVERLAPPEDWINDOW, 0, 0, 1920, 1080, nullptr, nullptr, wc.hInstance, nullptr);
    ...
    image_directory = utils::open_directory_dialog("niga");
    ...
    while (!done)
    {
...

        {
            ...
            const char* items[] = { "ct_body", "ct_head", "t_body", "t_head" };
            ...
            ImGui::SliderInt("Index", &image_idx, 0, images.size());
            ImGui::PopFont();
            ImGui::SameLine();
            ...
            if (key_pressed(VK_RETURN)) {
                if (!deleting_image) {
                    ...
                    else if (annotation_type == ct_head) {
                        annotation_type = ct_body;
                    }
                    if (annotation_type == t_head) {
                        annotation_type = t_body;
                    }
                    else if (annotation_type == t_body) {
                        annotation_type = t_head;
                    }
         ...
         ;
            if (!is_actually_drawing) {
                for (auto& annotation : images[image_idx].annotations) {
                    ...
                    std::cout << "X1: " << display_x1 << std::endl;
                    std::cout << "offset x " << get_scroll_and_zoom_offset(image_pos).x << std::endl;
                    ...
                    if (annotation.type == ct_body) {
                        ...
                        draw->AddText(ImVec2(display_x1 + 1, display_y1 - 13 + 1), IM_COL32(0, 0, 0, 255), "CT_BODY");
                        draw->AddText(ImVec2(display_x1, display_y1 - 13), IM_COL32(255, 255, 255, 255), "CT_BODY");
                        ...
                        draw->AddText(ImVec2(display_x2 + 1, display_y1 - 13 + 1), IM_COL32(0, 0, 0, 255), "CT_HEAD");
                        draw->AddText(ImVec2(display_x2, display_y1 - 13), IM_COL32(255, 255, 255, 255), "CT_HEAD");

                    }
                    else if (annotation.type == t_body) {
                        // draw->AddRectFilled(ImVec2(display_x1 - 1, display_y1 - 13 - 1), ImVec2(display_x1 + ImGui::CalcTextSize("BODY").x + 1, display_y1 - 13 + 1 + ImGui::CalcTextSize("BODY").y ), IM_COL32_BLACK, 2);
                        draw->AddText(ImVec2(display_x1 + 1, display_y1 - 13 + 1), IM_COL32(0, 0, 0, 255), "T_BODY");
                        draw->AddText(ImVec2(display_x1, display_y1 - 13), IM_COL32(255, 255, 255, 255), "T_BODY");

                    }
                    else if (annotation.type == t_head) {
                        // draw->AddRectFilled(ImVec2(display_x1 - 1, display_y1 - 13 - 1), ImVec2(display_x1 + 1 + ImGui::CalcTextSize("HEAD").x, display_y1 - 13 + ImGui::CalcTextSize("HEAD").y + 1), IM_COL32_BLACK, 2);
                        draw->AddText(ImVec2(display_x2 + 1, display_y1 - 13 + 1), IM_COL32(0, 0, 0, 255), "T_HEAD");
                        draw->AddText(ImVec2(display_x2, display_y1 - 13), IM_COL32(255, 255, 255, 255), "T_HEAD");
                    ...
                    if (!annotation.is_resizing) {
                        ...
                        if (ImGui::IsMouseHoveringRect(ImVec2(display_x1 - resize_padding, display_y1 - resize_padding), ImVec2(display_x1 + resize_padding, display_y1 + resize_padding))
                            ...
                            annotation.resize_corner = 0;
                            draw->AddCircle(ImVec2(display_x1, display_y1), 3, IM_COL32(255, 255, 255, 255), 10);
                        }
                            ...
                            || annotation.resize_corner == 1) {
                            annotation.resize_corner = 1;
                            draw->AddCircle(ImVec2(display_x2, display_y1), 3, IM_COL32(255, 255, 255, 255), 10);
                        }
                            ...
                            annotation.resize_corner = 2;
                            draw->AddCircle(ImVec2(display_x1, display_y2), 3, IM_COL32(255, 255, 255, 255), 10);
                        }
                            ...
                            annotation.resize_corner = 3;
                            draw->AddCircle(ImVec2(display_x2, display_y2), 3, IM_COL32(255, 255, 255, 255), 10);
                        }
                    }
